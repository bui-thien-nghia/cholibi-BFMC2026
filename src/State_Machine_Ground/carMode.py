...
class CarModeChanger:
    def __init__(self):
        ...
        # Vehicle state tracking for overtaking/tailing decisions
        self.has_moving_vehicle = False
        self.has_stationary_vehicle = False
        self.following_curve = False
        self.switches = {}  # Generic boolean switches for state management
    ...
    def change_state(self):
        ...
        if threshold_met('pedestrian') or threshold_met('cyclist'):
            ...
            if self._get_mode() == CarMode.TURN:
                self.cur_speed = CarSpeed.STOP
            elif self.has_moving_vehicle or self.has_stationary_vehicle:
        ...
        if turn_met(self) or threshold_met('roundabout_sign'):
            ...
            if self.has_stationary_vehicle:
                self.cur_mode = CarMode.OVERTAKING
            else:
                self.cur_mode = CarMode.TAILING
    ...
    def _update_switch(self, key, value):
        """Set a boolean switch for state management (e.g. 'overtaking_possible')"""
        self.switches[key] = value

    def record_vehicle_state(self, has_moving, has_stationary):
        """Record whether detected vehicles are moving or stationary"""
        self.has_moving_vehicle = has_moving
        self.has_stationary_vehicle = has_stationary
