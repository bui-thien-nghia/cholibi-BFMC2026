...
class LaneController:
    ...
    def get_waypoint_control(self, curr_x, curr_y, curr_theta, target_wp):
        """
        Calculates control to follow a global (x, y) waypoint.
        Uses the car's physical parameters and smoothing.
        """
        ...
        # 2. Angle to target in global frame
        target_angle = math.atan2(dy, dx)
        
        # 3. Angle error relative to current heading
        alpha = target_angle - curr_theta
        # Normalize to [-pi, pi]
        alpha = math.atan2(math.sin(alpha), math.cos(alpha))
        
        # 4. Pure Pursuit Steering Logic
        # For a waypoint follower, we treat the waypoint as the "lookahead" point
        # steer = atan( (2 * L * sin(alpha)) / dist )
        if dist < 0.01: return 0.0, self.min_speed, "REACHED"
        
        steer_rad = math.atan((2 * self.L * math.sin(alpha)) / dist)
        steer_rad = np.clip(steer_rad, -self.max_steer, self.max_steer)
        
        # 5. Smoothing (Reusing self.alpha)
        steer_rad = (self.alpha * steer_rad) + ((1 - self.alpha) * self.prev_steer)
        self.prev_steer = steer_rad
        
        # 6. Navigation Speed
        speed = self.max_speed * 0.6 # Constant comfortable speed for navigation
        
        return np.rad2deg(steer_rad), speed, "NAV"
